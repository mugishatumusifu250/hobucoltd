...
const UsersAdmin = () => {
  ...
  const displayAdmins = isSearching ? searchResults.filter((u) => u.role === 'admin') : admins;
  const displayClients = isSearching ? searchResults.filter((u) => u.role === 'client') : clients;
  const displayManagers = isSearching ? searchResults.filter((u) => u.role === 'manager') : managers;
  ...
  const renderUserTable = (list, label, total, onExport) => {
    const displayList = isSearching
      ? searchResults.filter((u) => u.role === label.toLowerCase().slice(0, -1))
      : list;
    const displayTotal = isSearching
      ? searchResults.filter((u) => u.role === label.toLowerCase().slice(0, -1)).length
      : total;

    return (
      <div className="table-card" style={label === 'Clients' ? { marginTop: '50px' } : label === 'Managers' ? { marginTop: '50px', marginBottom: '70px' } : {}}>
        <div className="table-header">
          <h3>{label} ({displayTotal})</h3>
        ...
        <div className="table-pagination">
          <div className="pagination-info">
            There is {displayTotal} Total {label}
          </div>
        </div>
      </div>
    );
